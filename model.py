...
# We define a `@contextmanager` for the RethinkDB connection.
@contextmanager
def connection():
  conn = None
  try:
...
# The app will use a table `blogpost` in the database `webpy`. 
# You can override the default by defining `RDB_DB` and `RDB_TABLE`
# env variables.  
